...
*   **多圖對應架構 (Image-Prompt Binding)**：
    *   為解決多張參考圖或底圖的描述歧義問題，系統不使用全域 Prompt 來描述個別圖片。
    *   **Image Node** 與 **3D Node** 輸出時，會將「圖片數據 (Base64)」與「節點內的描述文字」打包成一個物件。
    *   **資料結構範例**：
        ```json
        "images": [
          {
            ...
            "prompt": "A cat sitting on a sofa"  // <--- 綁定於該圖片的描述
          },
          {
            "type": "3d_snapshot",
            "src": "data:image/png;base64,...",
            "prompt": "Isometric view of a bedroom" // <--- 綁定於該模型的描述
          }
        ]
        ```
*   **流程**：當 `buildInstruction` 被觸發時，程式會從 Generator Node 開始，尋找所有連接到其 Input Socket 的線段。
*   針對每一條線段，追蹤到來源節點 (Source Node)，讀取該節點內部的 `<input>`, `<textarea>`, `<select>` 值。
*   最終產出一個包含所有參數的 JavaScript Object (JSON)。
...
*   **轉譯 API**:
    ...
    *   **Method**: `POST`
    *   **Method**: `POST`
    *   **Body**: `{ contents: [{ parts: [{ text: SYSTEM_PROMPT (Based on Mode) + USER_JSON }] }] }`
    *   **System Prompt**: 依據 Instruction Node 選擇的模式動態切換。
        *   `Precise`: 包含 "Nano Banana Guide" 與 Reference/Smart Paint 處理邏輯。
        *   `Simple`: 僅包含 "Direct Translator" 指令與強制保留 Image Prompt 的規則。
*   **生圖 API**:
    *   **URL**: `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image-preview:generateContent`
    *   **Method**: `POST`
    *   **Body**: `{ contents: [{ parts: [{ text: TRANSLATED_PROMPT }] }], generationConfig: { responseModalities: ["IMAGE"] } }`

### 4.3 外部設定檔 (External Configuration)
*   **機制**：系統啟動時，會自動從指定的 URL 讀取 JSON 設定檔，動態生成 Style, Layout, Camera 的下拉選單選項。
*   **來源 (Source)**：
    *   **GitHub (Primary)**: `https://raw.githubusercontent.com/.../styles.json` (等)
    *   **Local (Fallback)**: 若 GitHub 讀取失敗 (例如網路問題或檔案未上傳)，系統會自動降級讀取本地端的 `.json` 檔案。
*   **檔案結構**：
    *   `styles.json`: 定義風格分類與 Prompt。
    *   `layouts.json`: 定義長寬比與構圖 Prompt。
    *   `cameras.json`: 定義視角、光影、鏡頭 Prompt。
